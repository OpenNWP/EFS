...
analysis_hour_string_prev=$analysis_hour_prev
if [ $analysis_hour_prev -lt 10 ]
then
  analysis_hour_string_prev="0$analysis_hour_prev"
fi
run_id_previous="EFS_$analysis_year_prev$analysis_month_prev$analysis_day_prev$analysis_hour_string_prev"
...
# executing real2GAME
$real2game_home_dir/run.sh $model_target_id $model_src_id $nsoillays $n_layers $res_id $omp_num_threads $real2game_home_dir $background_file 1 $analysis_year $analysis_month $analysis_day $analysis_hour_string $model_home_dir
...
start_model=$(date +%s)
# executing the model
$model_home_dir/run_scripts/op.sh $omp_num_threads $delta_t_between_analyses_min 1 $run_id $run_span_min $model_home_dir $analysis_year $analysis_month $analysis_day $analysis_hour_string
end_model=$(date +%s)
diff_model=$(echo "$end_model - $start_model" | bc)

# clean-up of output directories
directory=$output_destination/model_output/surface/$analysis_hour_string"UTC"
source $efs_home_dir/sh/cleanup.sh
directory=$output_destination/model_output/pressure_levels/$analysis_hour_string"UTC"
source $efs_home_dir/sh/cleanup.sh
directory=$output_destination/maps/$analysis_hour_string"UTC"
source $efs_home_dir/sh/cleanup.sh

# copying the output to a directory that is accessible by the webserver
cp $model_home_dir/output/$run_id/*surface.nc $output_destination/model_output/surface/$analysis_hour_string"UTC"/
cp $model_home_dir/output/$run_id/*pressure_levels.nc $output_destination/model_output/pressure_levels/$analysis_hour_string"UTC"/
...
then
  ...
  then
    $model_home_dir/plotting/plot_maps_batch.sh $omp_num_threads 0 $map_plot_interval_early_min $figs_save_path/maps/$analysis_hour_string"UTC" $model_home_dir $run_id $((72*60))
    $model_home_dir/plotting/plot_maps_batch.sh $omp_num_threads $((72*60 + $map_plot_interval_late_min)) $map_plot_interval_late_min $figs_save_path/maps/$analysis_hour_string"UTC" $model_home_dir $run_id $run_span_min
  else
    $model_home_dir/plotting/plot_maps_batch.sh $omp_num_threads 0 $map_plot_interval_early_min $figs_save_path/maps/$analysis_hour_string"UTC" $model_home_dir $run_id $run_span_min
  fi
